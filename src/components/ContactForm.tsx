...
const ContactForm: React.FC = () => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const { error } = await supabase
                .from('Leads') // Targeting the 'Leads' table
                .insert([
                    {
    ...
    return (
        <form className="space-y-8" onSubmit={handleSubmit}>
            ...
            <div className="pt-4">
                <button
                    className="w-full cta-button bg-[var(--sincro-blue)] text-white text-xl font-semibold shadow-xl shadow-blue-500/20 hover:bg-blue-600 hover:shadow-blue-500/40 transform hover:-translate-y-1"
                    type="submit" // Ensure button is of type submit
                    disabled={submissionStatus === 'success'} // Disable button after successful submission
                >
